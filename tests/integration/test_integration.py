...
# NOTE: test_detect_post_valid_save_image lives in tests/deployment/, not here:
# it asserts image_url, which requires a real S3 PutObject to succeed. Under
# `sam local` the Lambda runs with dummy credentials and IMAGES_BUCKET_NAME is
# the unresolved CloudFormation logical ID ("ImagesBucket"), so the S3 write
# can never succeed and the test is only meaningful against the deployed stack.
